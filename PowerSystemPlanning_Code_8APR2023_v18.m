...
%% Global information
crit_time = 0.0085;                           %% duration of first demand block
blocks    = 20;     
...
while true                  % Infinite loop that will only be broken if we get to the finish line.
    ...
    [Investment,Output_G_exist,Output_G_cand,Cost,GEP_optimizer] = GEP_evaluate(GEP_optimizer,sOR,ramp_capacity,DBB_info,Renew_Cap);
    
    % Graph Findings
    graph_GEP(Output_G_exist,Output_G_cand,iterations,fst_study_horizon,"off",GEP_Load,DBB_info,sOR,ramp_capacity);   %% Graph a Report with the information of the recent findings.
    ...
    for t = 1:last_study_horizon-fst_study_horizon+1
        ...
        [Output_G_cand,Output_G_exist,MDM_marker,UCP_optimizer,MDM_Curtailment] = UCP_evaluate(renewables_gen,test_days.most_Demand,num2str(fst_study_horizon+t-1),Investment(:,t),Week_Chrono_MDM,UCP_optimizer,"Most Demand",DBB_info);
        ...
        close all hidden;        % Attempt to free up memory       
        % Evaluate the UCP optimizer
        [Output_G_cand,Output_G_exist,MNL_marker,UCP_optimizer,MNL_Curtailment] = UCP_evaluate(renewables_gen,test_days.most_NetLoad,num2str(fst_study_horizon+t-1),Investment(:,t),Week_Chrono_MNL,UCP_optimizer,"Most Net Load",DBB_info);
        % Decide feasibility in case feasible, graph.
        ...
        close all hidden;        % Attempt to free up memory
        % Evaluate the UCP optimizer
        [Output_G_cand,Output_G_exist,LRG_marker,UCP_optimizer,LRG_Curtailment] = UCP_evaluate(renewables_gen,test_days.least_Renew_gen,num2str(fst_study_horizon+t-1),Investment(:,t),Week_Chrono_LRG,UCP_optimizer,"Least Renewable Generation",DBB_info);
        % Decide feasibility in case feasible, graph.
        ...
        close all hidden;        % Attempt to free up memory        
        % Evaluate the UCP optimizer
        [Output_G_cand,Output_G_exist,LNL_marker,UCP_optimizer,LNL_Curtailment] = UCP_evaluate(renewables_gen,test_days.least_NetLoad,num2str(fst_study_horizon+t-1),Investment(:,t),Week_Chrono_LNL,UCP_optimizer,"Least Net Load",DBB_info);
        % Decide feasibility in case feasible, graph.
        ...
        close all hidden;        % Attempt to free up memory        
        % Evaluate the UCP optimizer
        [Output_G_cand,Output_G_exist,MRG_marker,UCP_optimizer,MRG_Curtailment] = UCP_evaluate(renewables_gen,test_days.most_Renew_gen,num2str(fst_study_horizon+t-1),Investment(:,t),Week_Chrono_MRG,UCP_optimizer,"Most Renewable Generation",DBB_info);
        % Decide feasibility in case feasible, graph.
        ...
        close all hidden;        % Attempt to free up memory        
        % Evaluate the UCP optimizer
        [Output_G_cand,Output_G_exist,RAN_marker,UCP_optimizer,RAN_Curtailment] = UCP_evaluate(renewables_gen,test_days.Random,num2str(fst_study_horizon+t-1),Investment(:,t),Week_Chrono_RAN,UCP_optimizer,"Random days",DBB_info);
        % Decide feasibility in case feasible, graph.
        ...
        switch flag_UCP_Stage
            case "First"                      % First stage of the UCP
                Case = 0;
                if Feasibility ~= 0 && ((Aggregate_Curtailment/sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 0.2 && sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) > 1) ||  sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.4) >= 1)
                    disp("Option 1")
                    Case = Case + 1;
                    Renew_Cap = 0.95*renew_cap_calc(Investment(:,t),renew_gen_block,iterations,DBB_info,GEP_Load,Renew_Cap,t);
                ...
                elseif (Aggregate_Curtailment/sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 0.2 && sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) > 1) || sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.4) >= 1
                    disp("Option 2")
                    Case = Case + 1;
                    Renew_Cap = 0.95*renew_cap_calc(Investment(:,t),renew_gen_block,iterations,DBB_info,GEP_Load,Renew_Cap,t);
                    disp("The amount of renewable energy has been constrained.")
                elseif Feasibility ~= 0
                    disp("Option 3")
                    Case = Case + 1;
                    if Feasibility_FirmPower ~= 0 && Feasibility_Ramping ~= 0
                        disp("System Operating Reserve is incremented from "+ num2str(sOR(t)*100) +"% to "+ num2str((sOR(t)+.1)*100)  +"%."+newline)
                        sOR(t) = sOR(t) + 0.1;
                        ramp_actual = ramp_calc(Investment(:,t),iterations,DBB_info);    % We give the input about, according to the last investment decision we made, we need at least that.
                        ramp_capacity(t) = ramp_actual*1.2;                 % But we increase it by 50%.
                        disp("The system ramping capability is increased from " + char(v.format(ramp_actual))+ " MW to "+ char(v.format(ramp_actual*1.5))+ " MW." +newline)
                    elseif Feasibility_FirmPower ~= 0 
                        disp("System Operating Reserve is incremented from "+ num2str(sOR(t)*100) +"% to "+ num2str((sOR(t)+.1)*100)  +"%."+newline)
                        sOR(t) = sOR(t) + 0.1;
                    elseif Feasibility_Ramping ~= 0
                        ramp_actual = ramp_calc(Investment(:,t),iterations,DBB_info);    % We give the input about, according to the last investment decision we made, we need at least that.
                ...
                if and(t == last_study_horizon - fst_study_horizon + 1 , Case == 0)
            ...
            case "Second"                  % Second stage of the UCP
                Case = 0;
                if (Aggregate_Curtailment/sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 0.2 && sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) > 1) && sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.3) >= 1
                    disp("Option 1")
                    Case = Case + 1;
                    Renew_Cap = 0.975*renew_cap_calc(Investment(:,t),renew_gen_block,iterations,DBB_info,GEP_Load,Renew_Cap,t);
                    disp("The amount of renewable energy has been constrained.")
                    ramp_actual = ramp_calc(Investment(:,t),iterations,DBB_info);    % We give the input about, according to the last investment decision we made, we need at least that.
                    ramp_capacity(t) = ramp_actual*1.2;                 % But we increase it by 50%.
                    disp("The system ramping capability is increased from " + char(v.format(ramp_actual))+ " MW to "+ char(v.format(ramp_actual*1.5))+ " MW." +newline)
                elseif sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.3) >= 1
                    disp("Option 2")
                    Case = Case + 1;
                    Renew_Cap = 0.975*renew_cap_calc(Investment(:,t),renew_gen_block,iterations,DBB_info,GEP_Load,Renew_Cap,t);
                    disp("The amount of renewable energy has been constrained.")
...
function Renew_Cap = renew_cap_calc(Investment,renew_gen_block,iterations,DBB_info,GEP_Load,Renew_Cap,t)
    ...
    if iterations == 0
        ...
        counter = 1;
        for i = Flags.flag_EcoThermal_exist+1:Flags.flag_renewables_exist
            Measure_Cap = Measure_Cap + sum(8760*Exist_Upper(i).*renew_gen_block.(string(Renewables_exist_data.RenewableScenario{counter})).*GEP_Load.LD);
            counter = counter +1;
        end
        % Candidate Renewables
        counter = 1;
        for i = Flags.flag_EcoThermal_cand+1:Flags.flag_renewables_cand
            Measure_Cap = Measure_Cap + sum(Investment(i)*8760*Cand_Upper(i).*renew_gen_block.(string(Renewables_cand_data.RenewableScenario{counter})).*GEP_Load.LD);
            counter = counter + 1;
        end
        % Measurement of Renew Cap
        Renew_Cap(t) = min(Measure_Cap,Renew_Cap(t));
    end
end
