...
% Directory
Database_name  = "StochasticTaiwan";           %% Change name depending on which information we want to work with.
...
scenarios       = 2;            % Number of renewable scenarios
...
while true                  % Infinite loop that will only be broken if we get to the finish line.
    ...
    for t = 1:last_study_horizon-fst_study_horizon+1
        for s = 1:scenarios
            ...
            % Feebdback for the GEP
            switch flag_UCP_Stage
                case "First"                      % First stage of the UCP
                    Case = 0;
                    if Feasibility ~= 0 && ((Aggregate_Curtailment/sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 0.2 && sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) > 1) || (sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 1 &&  sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.4) >= 1))
                        disp("Option 1")
                        Case = Case + 1;
                        Renew_Cap = renew_cap_calc(Investment(:,t),renew_gen_block.("S"+s).("Y"+num2str(fst_study_horizon+t-1)),iterations,DBB_info,GEP_Load,Renew_Cap,t,0.95);
                        if Feasibility_FirmPower ~= 0
                            disp("System Operating Reserve is incremented from "+ num2str(sOR(t)*100) +"% to "+ num2str((sOR(t)+.1)*100)  +"%."+newline)
                            sOR(t) = sOR(t) + 0.1;
                            break
                        elseif Feasibility_Ramping ~= 0
                            ...
                            ramp_capacity(t) = ramp_actual*1.2;                 % But we increase it by 50%.
                    ...
                    elseif ((Aggregate_Curtailment/sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 0.2 && sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) > 1) || (sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 1 &&  sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.4) >= 1))
                        disp("Option 2")
                        Case = Case + 1;
                        Renew_Cap = renew_cap_calc(Investment(:,t),renew_gen_block.("S"+s).("Y"+num2str(fst_study_horizon+t-1)),iterations,DBB_info,GEP_Load,Renew_Cap,t,0.95);
                        break
                    elseif Feasibility ~= 0
                        disp("Option 3")
                        Case = Case + 1;
                        if Feasibility_FirmPower ~= 0 
                            disp("System Operating Reserve is incremented from "+ num2str(sOR(t)*100) +"% to "+ num2str((sOR(t)+.1)*100)  +"%."+newline)
                        ...
                        elseif Feasibility_Ramping ~= 0
                            ramp_actual = ramp_calc(Investment(:,t),iterations,DBB_info);    % We give the input about, according to the last investment decision we made, we need at least that.
                            ramp_capacity(t) = ramp_actual*1.2;                 % But we increase it by 50%.
                            disp("The system ramping capability is increased from " + char(v.format(ramp_actual))+ " MW to "+ char(v.format(ramp_actual*1.2))+ " MW." +newline)
                            break
                        end
                    end
            
                    % We change to enter the second stage of the verifications
                    if and(t == last_study_horizon - fst_study_horizon + 1 , Global_Feasibility == 0)
                        flag_UCP_Stage = "Second";      % We change the flag to start the second stage of the UCP verification
                        Aggregate_problems = nan;       % We create another flag that exists after the first stage of the UCP. With this flag we will keep track of the problems of the UCP in the second stage.
                    elseif t == last_study_horizon - fst_study_horizon + 1
                        Global_Feasibility = 0;
                    end                                 % The while loop will only end after there is no problems in the first stage.
                case "Second"                  % Second stage of the UCP
                    Case = 0;
                    if Feasibility_FirmPower ~= 0
                        sOR(t) = sOR(t) + 0.1;
                        break;
                    else
                        if (Aggregate_Curtailment/sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 0.2 && sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) > 1) && (sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 1 &&  sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.3) >= 1)
                            disp("Option 1")
                            Case = Case + 1;
                            Aggregate_problems = Aggregate_problems + 1;        % Indicates that it identified a problem in a year
                            Renew_Cap = renew_cap_calc(Investment(:,t),renew_gen_block.("S"+s).("Y"+num2str(fst_study_horizon+t-1)),iterations,DBB_info,GEP_Load,Renew_Cap,t,0.975);
                            ramp_actual = ramp_calc(Investment(:,t),iterations,DBB_info);    % We give the input about, according to the last investment decision we made, we need at least that.
                            ramp_capacity(t) = ramp_actual*1.2;                 % But we increase it by 20%.
                            disp("The system ramping capability is increased from " + char(v.format(ramp_actual))+ " MW to "+ char(v.format(ramp_actual*1.2))+ " MW." +newline)
                            break
                        elseif sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 1 &&  sum([MDM_Curtailment;MNL_Curtailment;LRG_Curtailment;LNL_Curtailment;MRG_Curtailment] >= 0.3) >= 1
                            disp("Option 2")
                            Case = Case + 1;
                            Aggregate_problems = Aggregate_problems + 1;        % Indicates that it identified a problem in a year
                            Renew_Cap = renew_cap_calc(Investment(:,t),renew_gen_block.("S"+s).("Y"+num2str(fst_study_horizon+t-1)),iterations,DBB_info,GEP_Load,Renew_Cap,t,0.975);
                            break
                        elseif Aggregate_Curtailment/sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) >= 0.2 && sum([MDM_marker;MNL_marker;LRG_marker;LNL_marker;MRG_marker;RAN_marker] == 0) > 1
                            disp("Option 3")
                            Case = Case + 1;
                            Aggregate_problems = Aggregate_problems + 1;        % Indicates that it identified a problem in a year
                            ramp_actual = ramp_calc(Investment(:,t),iterations,DBB_info);    % We give the input about, according to the last investment decision we made, we need at least that.
    ...
    if exist('Aggregate_problems','var')  %% Termination Criteria
        if Aggregate_problems == 0 && Global_Feasibility == 0
        ...
        elseif isnumeric(Aggregate_problems) && Global_Feasibility == 0
            Aggregate_problems = 0;
        elseif Global_Feasibility ~= 0
            Global_Feasibility = 0;
            flag_UCP_Stage = "First";       % We go back to our First Stage. We added this because we identified that it might happen that we suddenly have unfeasible cases back again. We only declare that we must go back to the initial case, and increase sOR. No further action is taken.
            Aggregate_problems = nan;       % We return Aggregate problems to its initial condition. This will prevent from terminating the loop.
        end
    end
end
...
function GEP_optimizer = GEP_optimizer_creator(GEP_Load,Renew_commitment,renew_gen_block,DBB_info,opt_year)
    ...
    for s = 1:scenarios
        %                                                                                                 2050                                                                                                                                  2050                                                                                               2050         
        Constraints = Constraints + [[sum(Cand_git(Flags.flag_thermal_cand+1:Flags.flag_renewables_cand,:,str2double(opt_year)-fst_study_horizon+1,s)*(GEP_Load.LD)) + sum(Exist_git(Flags.flag_thermal_exist+1:Flags.flag_renewables_exist,:,str2double(opt_year)-fst_study_horizon+1,s)*(GEP_Load.LD)) >= Renew_commitment*GEP_Load.LD'*GEP_Load.(opt_year)]: 'Renewable Quota 2050']; 
        %                                                                                                 2025                                                                                         2025                   20% is the energy commitment for that year according to: https://www.moea.gov.tw/MNS/english/Policy/Policy.aspx?menu_id=32904&policy_id=19
        Constraints = Constraints + [[sum(Cand_git(Flags.flag_EcoThermal_cand+1:Flags.flag_renewables_cand,:,5,s)*(GEP_Load.LD)) + sum(Exist_git(Flags.flag_EcoThermal_cand+1:Flags.flag_renewables_exist,:,5,s)*(GEP_Load.LD)) >= 0.2*GEP_Load.LD'*GEP_Load.("2025")]: 'Renewable Quota 2025']; 
        %                                                                                                 2030                                                                                         2030                   25 is the energy commitment for that year according to: https://www.ndc.gov.tw/en/nc_8455_36526
        Constraints = Constraints + [[sum(Cand_git(Flags.flag_EcoThermal_cand+1:Flags.flag_renewables_cand,:,10,s)*(GEP_Load.LD)) + sum(Exist_git(Flags.flag_EcoThermal_cand+1:Flags.flag_renewables_exist,:,5,s)*(GEP_Load.LD)) >= 0.25*GEP_Load.LD'*GEP_Load.("2030")]: 'Renewable Quota 2030']; 
    end
